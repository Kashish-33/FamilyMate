...
function Chat() {
  ...
  const handleSend = async (messageToSend = input) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Could not get a response from FamilyMate.");
    } finally {
      setIsSending(false);
    }
  };
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5 sm:p-7">
          {messages.length === 0 ? (
            <div className="mx-auto flex h-full max-w-2xl flex-col items-center justify-center py-12 text-center">
              ...
              <h2 className="mt-5 text-2xl font-bold text-slate-800">Hello, I&apos;m your FamilyMate</h2>
            ...
            <div className="mx-auto max-w-3xl space-y-5">
              ...
              {isSending && <div className="flex justify-start"><div className="rounded-3xl rounded-bl-md bg-[#FFF4F6] px-5 py-3.5 text-sm text-slate-500">FamilyMate is thinking...</div></div>}
            </div>
          )}
